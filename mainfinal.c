...
int main(){
	...
	drawbricks(stdscr,bricks,ymax/3,xmax/3,NoRows,NoColumns,&xstart,&xend);
	refresh();
	object ball;
	ball.r.x = xmax/2;
	ball.r.y = ymax - 2;
	...
	while(1){
		...
		while(1){
			if(powerups == 1){
				crash = checkforcollision(bricks,&powerup,xstart,xend,NoRows,NoColumns,&bullet);
				if(crash == 1){
			...
			if(feature_alive == 0 && feature == 0 && rand()%50){      //rand()%50 last argument as it is most expensive
				powerup.r.x = rand()%xmax;
			...
			tempy = ball.r.y;
			tempx = ball.r.x;	
			crash = checkforcollision(bricks,&ball,xstart,xend,NoRows,NoColumns,&dummy);
			if(crash == 1){
				attron(COLOR_PAIR(11));
				score = score + 25;
				mvprintw(2,xmax/2 - 4,"Score: %d",score);
